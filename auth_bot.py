...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Display info about the authorized user."""
    user_id = update.effective_user.id
    ...
    try:
        async with create_telegram_client(session_name) as client:
            if await client.is_user_authorized():
                ...
                await update.message.reply_text(
                    f"Authorized user: {me.first_name} {me.last_name or ''}\n"
                    f"Username: @{me.username or 'N/A'}\n"
                    f"Phone: {me.phone}"
                )
            else:
                await update.message.reply_text("Not authorized yet. Use /authorize to log in.")
    except Exception as e:
