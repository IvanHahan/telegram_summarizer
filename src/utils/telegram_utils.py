...
class TelegramHandler:
    """
    A handler class that manages Telegram client operations.
    """
    
    def __init__(self, user_id: str):
        """
        Initialize the TelegramHandler with a user ID.
    ...
    def _ensure_client(self):
        ...
        if self.client is None:
            ...
            self.client = TelegramClient(self.user_id, api_id, api_hash)
            ...
            if dialog.unread_count > 0:  # Check if the chat has unread messages
                ...
                if (include_private and dialog.is_user) or \
                   ...
                   (include_channels and dialog.is_channel):
                    ...
                    for message in self.client.iter_messages(dialog.id, limit=limit_messages):
    ...
    def get_chat_history(self, chat_id, hours=10, max_words: int = None):
        """
    ...
    def mark_chats_as_read(self, chat_ids):
        """
        ...
        for chat_id in chat_ids:
            self.client.send_read_acknowledge(chat_id)

    def bigram_similarity(self, query: str, target: str) -> float:
        """
    ...
    def search_chat(self, query, top_k=3):
        """
...
# Helper functions for backward compatibility and factory methods
def create_telegram_handler(user_id: str) -> TelegramHandler:
    """
    Create and return a TelegramHandler instance.
    
    Args:
        user_id (str): The user ID to create a session for.
    
    Returns:
        TelegramHandler: An instance of the TelegramHandler.
    """
    return TelegramHandler(user_id)
...
def is_authorized(user_id: str):
    """Check if a user is authorized (backward compatibility function)."""
    handler = TelegramHandler(user_id)
    with handler:
        return handler.is_authorized()


# Backward compatibility functions - these use the TelegramHandler internally
def get_unread_chats(client, **kwargs):
    """Backward compatibility function."""
    # This is a bit tricky since we need to extract user_id from the client
    # For now, we'll raise an error suggesting to use TelegramHandler directly
    raise DeprecationWarning("Use TelegramHandler class instead of standalone functions")


def get_chat_history(client, chat_id, **kwargs):
    """Backward compatibility function."""
    raise DeprecationWarning("Use TelegramHandler class instead of standalone functions")


def mark_chats_as_read(client, chat_ids):
    """Backward compatibility function."""
    raise DeprecationWarning("Use TelegramHandler class instead of standalone functions")


def search_chat(client, query, **kwargs):
    """Backward compatibility function."""
    raise DeprecationWarning("Use TelegramHandler class instead of standalone functions")
...
if __name__ == "__main__":
    # Example usage with TelegramHandler
    ...
    with create_telegram_handler(user_id) as handler:
        ...
        unread_private_chats = handler.get_unread_chats(
            include_private=True, include_groups=False, include_channels=False, 
            include_muted=False, max_unread_count=5
        )
        ...
        unread_group_chats = handler.get_unread_chats(
            include_private=False, include_groups=True, include_channels=False
        )
        print("\nUnread group chats:")
        ...
        unread_channel_chats = handler.get_unread_chats(
            include_private=False, include_groups=False, include_channels=True
        )
        print("\nUnread channels:")
        ...
        unread_all_chats = handler.get_unread_chats(
            include_private=True, include_groups=True, include_channels=True, include_muted=True
        )
